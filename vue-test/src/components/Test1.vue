    ...
    <table class="table ui-accordion">
      ...
      <tbody>
        <template v-for="row in data.rows">
          <tr class="head" @click="toggle(row.title)" :key="`${row.title.order}_row}`">
            <td>{{ row.title.order }}</td>
            <td>{{ row.title.type }}</td>
            <td>{{ row.title.question }}</td>
          </tr>
          <tr class="body " :class="bindClass(row.title)" :key="`${row.title.order}_content}`" >
            <td colspan="3">
...
export default {
  ...
  methods: {
    toggle: function(selectRow) {
      this.data.rows.forEach(row => {
        if (row.title.order !== selectRow.order) {
          row.title.isActive = false
        }        
      });
      selectRow.isActive = !selectRow.isActive
    },
    bindClass: function(selectRow) {
      return selectRow.isActive ? 'active' : ''
    }
  }
}
</script>
